...
class ClassifyData:
    """Input processed data to classify variants into: Somatic, Ambiguous,
    and Fail
    """

    def __init__(self, solid_tumor, classifier, samples_file_path,
                 header, output_dir_path):
        """Assemble pandas.Dataframe of data
            ...
            Args:
                ...
                classifier (str): path for classifier
                                  (default is 'deep_learning_classifier.json')
    ...
    def _classify_samples(self, solid_tumor, classifier, output_dir_path):
        """classify processed data using classifier
            ...
            Args:
                solid_tumor (bool): True if solid tumor, False otherwise
                                    (i.e. hematologic tumor).
                classifier (str): path for classifier (default is
                                  'deep_learning_classifier.json')
                out_dir_path (str): path for output

        """
        estimator = model_from_json(open('deep_learning_classifier.json'
                                         ).read())
        ...
        X = processed_data.sort_index(axis=1).drop(['disease_BRC',
                                                    'reviewer_1'], axis=1
                                                   ).astype(float).values
